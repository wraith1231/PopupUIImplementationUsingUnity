...
public class ShopSphere : MonoBehaviour
{
    public static int POPUP = Animator.StringToHash("Popup");
    public static int PAUSE = Animator.StringToHash("Pause");
    ...
    IEnumerator PopupCoroutine()
    {
        ...
        if (_isOpen)
        {
            ItemData data = SingletonManager.GetRandomItem();
            _rawImage.texture = data.texture;
            _title.text = data.title;
            _description.text = data.description;
            _price.text = $"Price : {data.price}$";
            _rate.text = $"Rate : {data.rating.rate}\nReviews : {data.rating.count}";

            Debug.Log(data.rating.count);
            if(data.rating.count > 200)
            {
                if (data.rating.rate > 4.5)
                {
                    _price.color = Color.yellow;
                    _rate.color = Color.yellow;
                }
                else if (data.rating.rate < 3.0)
                {
                    _price.color = Color.red;
                    _rate.color = Color.red;
                }
                else
                {
                    _price.color = Color.white;
                    _rate.color = Color.white;
                }
            }
            else
            {
                _price.color = Color.blue;
                _rate.color = Color.blue;
            }
        }
        ...
        while (true)
        {
            ...
            {
                if (_animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 0.00f)
                {
                    _animator.speed = 0f;
                    break;
                }
